...
import { signupPage, embedPage, adminPage, messagePage, unsubscribePage, creditUrl } from "./html";
...
type OutgoingEmail = { to: string; subject: string; html: string; headers: Record<string, string> };
...
const creditMode = (env: Bindings) => String(env.SHOW_CREDIT ?? "").trim().toLowerCase();
const creditOnPages = (env: Bindings) => !["false", "email"].includes(creditMode(env));
const creditInEmail = (env: Bindings) => !["false", "pages"].includes(creditMode(env));
...
const applyMergeTags = (origin: string, html: string, token: string, email: string, name?: string | null) =>
  html
...
async function enqueueCampaign(
  ...
  ).bind(subject, html, baseUrl).first<{ id: number }>();
...
async function drainOutbox(env: Bindings): Promise<void> {
  ...
  ).bind(sendBatchSize(env)).all<OutboxRow>();
  ...
  ).bind(...ids).all<{ id: number; subject: string; body_html: string | null; base_url: string | null }>();
  ...
  const sendIndividually = async (list: { row: OutboxRow; msg: OutgoingEmail }[]) => {
    for (const d of list) {
      try {
  ...
  if (sendEmailBatch && deliverable.length) {
    ...
    try {
      await sendEmailBatch(env, deliverable.map((d) => d.msg));
      sent.push(...deliverable.map((d) => d.row));
    } catch (err) {
      console.error("sendEmailBatch failed, falling back to per-email delivery:", err);
...
app.get("/", (c) => c.html(signupPage(c.env.TURNSTILE_SITE_KEY, c.env.PRIVACY_URL, pageCredit(c.env, "signup"))));

// --- Public: bare form for iframe/script embedding on your own site ---
app.get("/embed", (c) => c.html(embedPage(c.env.TURNSTILE_SITE_KEY, c.env.PRIVACY_URL, pageCredit(c.env, "embed"))));
...
app.post("/api/subscribe", async (c) => {
  ...
  if (doubleOptIn) {
    ...
    const row = await c.env.DB.prepare(
      ...
      .first<{ status: string; confirm_token: string | null; unsub_token: string }>();
...
app.get("/confirm", async (c) => {
  ...
  return c.html(messagePage("You're subscribed!", "Thanks for confirming — you're all set.", pageCredit(c.env, "confirm")));
});
...
app.get("/unsubscribe", (c) => {
  const token = c.req.query("t") || c.req.query("token") || "";
  if (!token) return c.html(messagePage("Invalid link", "This unsubscribe link is incomplete.", pageCredit(c.env, "unsubscribe")));
...
app.post("/unsubscribe", async (c) => {
  ...
  return (c.req.header("accept") || "").includes("text/html")
    ? c.html(messagePage("You've been unsubscribed.", "You won't receive further emails.", pageCredit(c.env, "unsubscribed")))
    : c.text("unsubscribed");
});
...
function postEmailHtml(item: FeedItem): string {
  const text = (item.summary || "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
  const excerpt = text ? `<p>${text.slice(0, 300)}${text.length > 300 ? "…" : ""}</p>` : "";
...
async function runAutoSend(env: Bindings): Promise<void> {
  ...
  const { results } = await env.DB.prepare(`SELECT item_id FROM sent_posts`).all<{ item_id: string }>();
  const seen = new Set(results.map((r) => r.item_id));
  const fresh = items.filter((i) => !seen.has(i.id));
  if (!fresh.length) return;

  const markSeen = (id: string) =>
    env.DB.prepare(`INSERT OR IGNORE INTO sent_posts (item_id) VALUES (?1)`).bind(id).run();
  ...
  for (const item of fresh.reverse()) {
    await enqueueCampaign(env, baseUrl, item.title || "New post", postEmailHtml(item));
